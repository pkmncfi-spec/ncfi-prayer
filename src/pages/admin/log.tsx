...
export default function NotificationPage() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      const notificationsRef = collection(db, "logs");
        ...
        <div className="flex-1 overflow-y-auto mt-[80px] px-2">
          {notifications.length === 0 ? (
            ...
            notifications.map((notification) => (
              <div
                onClick={() => {notification.type === "devotion" ? router.push(`/admin/devotion/${notification.postId}`) : router.push(`/admin/post/${notification.postId}`)}}
                key={notification.id}
