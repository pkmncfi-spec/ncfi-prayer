...
export default function PostPage() {
    ...
    const deletePost = async (postId: string) => {
        try {
            await updateDoc(doc(db, "posts", postId), { forInternational: false });
            await addDoc(collection(db, "notifications"), {
                title: "Your prayer request",
                    ...
                    <DialogContent className={`${GeistSans.className} max-w-[350px]`}>
                        <DialogDescription className="mt-6">
                            <DialogClose className="w-full">
                                    <Button variant={"outline"} onClick={() => acceptPrayer(postId as string)} className="h-14 border-gray-500 text-xl w-full mb-6 hover:bg-gray-200 active:bg-primary/30">
                                    <FaRegCheckSquare /> Accept Prayer
                                    </Button>
                                    ...
                                    <FaRegWindowClose /> Reject Prayer
                                    </Button>
                            </DialogClose>
                        </DialogDescription>
                    </DialogContent>
